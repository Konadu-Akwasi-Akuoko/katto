...
pub async fn transcribe(cfg: &TranscribeConfig, wav_path: &Path) -> Result<(Vec<u8>, Transcript)> {
    ...
    match attempt(cfg, bytes.clone()).await {
        Err(Retry::After(_)) => {}
        Err(Retry::Fatal(e)) => return Err(e),
        Ok(ok) => return Ok(ok),
    }
    tokio::time::sleep(RETRY_DELAY).await;
    match attempt(cfg, bytes).await {
        Ok(ok) => Ok(ok),
...
async fn attempt(
    ...
    match status.as_u16() {
        ...
        401 => Err(Retry::Fatal(Error::TranscribeAuth(body_text()))),
...
pub async fn transcribe_into_bundle(
    ...
    write_atomic(&bundle_root.join(TRANSCRIPT_JSON), &raw)?;
    Ok(transcript)
}
