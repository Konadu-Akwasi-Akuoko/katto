...
Data: app-support SQLite (index only) · Studio root on external SSD (folders = truth)
```
...
| D5 | **Project = one video = a folder first, DB row second.** Folder is truth; SQLite only indexes (rescan/reconcile on launch; a folder deleted in Finder disappears from the app). Projects live under a **Studio root on an external SSD** chosen at onboarding; unmounted root shows an explicit "drive disconnected" state. | If katto died, every project remains a clean, self-explanatory folder. |
| D6 | **Folder anatomy:** `project.json`, `footage/` (**flat** — no a-cam/b-cam split; `YYYY-MM-DD_NNN.ext` naming), `audio/`, `assets/{envato,vfx,graphics}`, `thumbnails/`, `timelines/` (versioned `-vN`, never overwritten), `exports/`. `assets/vfx/` is engine-agnostic (HyperFrames, Remotion, AE — one folder per effect: project file + render). The `.kruproj` cut bundle lives **inside** the owning project folder; loose-bundle open stays supported. | Owner decision on flatness; engine-agnostic VFX keeps the NLE referencing plain rendered files. |
...
| D20 | **Onboarding:** Studio root picker, ElevenLabs key (keychain), `claude` PATH detection, default-NLE preference. **Packaging:** `.dmg` via Tauri bundler, launch-at-login; signing/notarization deferred until distribution matters (Apple Developer ID, $99/yr). | First-run must establish the four things katto cannot guess. |
