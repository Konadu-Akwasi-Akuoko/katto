		...
		<section className="flex flex-col gap-6">
			<div className="flex flex-col gap-2">
				...
				<p className="text-fg-muted">
					Cut planning talks to Claude — through the claude CLI when it's
					installed, or the API with an Anthropic key. Neither is needed until
					the first rough cut.
				</p>
			</div>
			{detection.isPending && (
				<p className="text-sm text-fg-muted">
					Looking for claude on your PATH…
				</p>
			)}
			{detection.data != null && (
				<p className="text-sm">
			...
			{!detection.isPending && detection.data == null && (
				<div className="flex flex-col gap-3">
					<p className="text-sm text-fg-muted">
						claude isn't on your PATH. Paste an Anthropic key instead, or sort
						it out later in Settings.
					</p>
					{keyStored ? (
